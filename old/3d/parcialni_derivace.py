...
outputfile="parcialni_derivace_"+str(priklad)+".png" # jmeno vystupniho souboru
...
ax.plot(xxx, [ymax_axes for i in xxx], zzzy, color='red')
ax.plot([x0 for i in yyy], yyy, zzzx, color='blue')
ax.plot([xmin_axes for i in yyy], yyy, zzzx, color='blue')
ax.scatter(x0,y0,f(x0,y0)+0.3, c='r', zorder=100, s=100)

print f(x0,y0)

ax.set_xlabel('x')
ax.set_xlim(xmin_axes, xmax_axes)
ax.set_ylabel('y')
ax.set_ylim(ymin_axes, ymax_axes)
ax.set_zlabel('f(x,y)')
